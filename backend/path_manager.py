...
class PathManager:
    ...
    def _get_project_root(self):
        """获取项目根目录路径"""
        def getRootDir(start_dir):
            current_dir = os.path.abspath(start_dir)

            # 检查当前目录是否已经是根目录
            if os.path.basename(current_dir) == "CosyVoice_app":
                if os.path.exists(os.path.join(current_dir, "backend")):
                    return current_dir

            # 检查子目录
            cosyvoice_app_dir = os.path.join(current_dir, "CosyVoice_app")
            if os.path.exists(cosyvoice_app_dir):
                if os.path.exists(os.path.join(cosyvoice_app_dir, "backend")):
                    return cosyvoice_app_dir

            # 递归向上查找
            parent_dir = os.path.dirname(current_dir)
            if parent_dir == current_dir:
                return None
            return getRootDir(parent_dir)

        result = getRootDir(os.getcwd())
        if result is None:
            raise FileNotFoundError("无法找到 CosyVoice_app 项目根目录")
        return result
    ...
    def get_temp_voice_path(self, base_name):
        ...
        return os.path.join(
            self.project_root,
            f"temp_{base_name}_{int(datetime.now().timestamp())}.wav"
        )
